...
try:
    ...
    print("Warning: groq library not installed. AI enhancement disabled.")

# ---------------------------------------------------------------------------
# AI Enhancer (unchanged)
# ---------------------------------------------------------------------------
class AIDocumentationEnhancer:
    """Enhances documentation using AI (Groq API)"""

    def __init__(self, config):
        self.enabled = config.get('enabled', False) and GROQ_AVAILABLE
...
class MarkdownGenerator:
    ...
    def generate(self, file_path, info, root_dir, repo_name, code_content=''):
        # Skip empty files or files with no significant info
        if not info.get('classes') and not info.get('functions') and not info.get('routes') and not info.get('summary'):
            return False
...
def main():
    ...
    if os.path.exists(output_dir):
        try:
            ...
            print(f"Warning: Could not remove {output_dir} due to permission error. Continuing without deletion.")
